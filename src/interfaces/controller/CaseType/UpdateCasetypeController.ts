...
export class UpdateCasetypeController implements IController {
    constructor(
        private _updateCasetypeUsecase: IUpdateCaseTypeUsecase,
    ...
    async handle(httpRequest: HttpRequest): Promise<IHttpResponse> {
        ...
        if (!id.trim()) {
            return this._httpErrors.error_400("no id is found.");
        }
        if (!name.trim()) {
            return this._httpErrors.error_400("name is not required");
        }
        if (!practiceId) {
            return this._httpErrors.error_400("practice id is required");
        }
        try {
            const result = await this._updateCasetypeUsecase.execute({
                id,
                name,
