...
class PdfReportGenerator {
    async generateReport(data, summary) {
        return new Promise((resolve, reject) => {
            const doc = new pdfkit_1.default({ margin: 10 });
            const chunks = [];
            ...
            data.forEach((row) => {
                const trimmedLawyerId = row.lawyerId.length > 12
                    ? row.lawyerId.substring(0, 12) + "…"
                    : row.lawyerId;
                const trimmedClientId = row.clientId.length > 12
    ...
    drawTableHeader(doc, y, colWidths, rowHeight) {
    ...
    drawRow(doc, y, rowHeight, rowData, colWidths) {
