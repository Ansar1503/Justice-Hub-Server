...
class UserRepository {
    Mapper;
    _session;
    constructor(Mapper = new UserMapper_1.UserMapper(), _session) {
        this.Mapper = Mapper;
    ...
    async create(user) {
        ...
        const savedUser = await new UserModel_1.default(mapped)
            .save({ session: this._session });
        return this.Mapper.toDomain(savedUser);
    }
    async findByEmail(email) {
        const userDoc = await UserModel_1.default.findOne({ email });
        return userDoc ? this.Mapper.toDomain(userDoc) : null;
    }
    async findByuser_id(user_id) {
        const userDoc = await UserModel_1.default.findOne({ user_id });
        return userDoc ? this.Mapper?.toDomain(userDoc) : null;
    }
    async update(user) {
        ...
        const updated = await UserModel_1.default.findOneAndUpdate(query, update, {
            new: true,
        });
    ...
    async findAll(query) {
        ...
        dataPipeline.push({ $project: { password: 0 } }, { $sort: sortStage }, { $skip: (page - 1) * limit }, { $limit: limit });
        countPipeline.push({ $count: "total" });
        const [users, countResult] = await Promise.all([
            UserModel_1.default.aggregate(dataPipeline),
            UserModel_1.default.aggregate(countPipeline),
        ]);
    ...
    async findLawyersByQuery(query) {
        ...
        dataPipeline.push(...lookupStage, { $match: matchStage2 }, { $project: { password: 0 } }, { $sort: sortStage }, { $skip: (page - 1) * limit }, { $limit: limit });
    ...
    async countTotalClients() {
        return await UserModel_1.default.countDocuments({ role: "client" });
    }
    async countTotalLawyers() {
        return await UserModel_1.default.countDocuments({ role: "lawyer" });
    }
}
