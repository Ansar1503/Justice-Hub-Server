...
const ChatSessionMapper_1 = require("@infrastructure/Mapper/Implementations/ChatSessionMapper");
const ChatSessionModel_1 = require("../model/ChatSessionModel");
class ChatSessionRepository {
    mapper;
    constructor(mapper = new ChatSessionMapper_1.ChatSessionMapper()) {
        this.mapper = mapper;
    }
    async create(payload) {
        const chat = new ChatSessionModel_1.ChatModel(this.mapper.toPersistence(payload));
        await chat.save();
    ...
    async aggregate(payload) {
        ...
        const results = await ChatSessionModel_1.ChatModel.aggregate(pipeline);
    ...
    async update(payload) {
        ...
        const updatedChatSession = await ChatSessionModel_1.ChatModel.findOneAndUpdate({ _id: payload.id }, update, { new: true });
        if (!updatedChatSession)
            return null;
        return this.mapper.toDomain(updatedChatSession);
    }
    async findById(id) {
        const data = await ChatSessionModel_1.ChatModel.findOne({ _id: id });
        return data ? this.mapper.toDomain(data) : null;
    }
    async findByUserId(userId) {
        const data = await ChatSessionModel_1.ChatModel.find({
            $or: [
                { "participants.lawyer_id": userId },
