...
export class CreateCheckoutSessionUseCase implements ICreateCheckoutSessionUseCase {
    ...
    async execute(input: CreateCheckoutSessionInputDto): Promise<any> {
        ...
        if (lawyerVerificaitionDetails.verificationStatus !== "verified") throw new Error(ERRORS.LAWYER_NOT_VERIFIED);
        ...
        if (numberOfCancel && numberOfCancel >= 2) throw new Error("Number of cancellation exceeded");
        if (bookingExist) {
        ...
        if (override && Object.keys(override).length > 0) {
            ...
            if (overrideDate.timeRanges.length > 0) {
                ...
                if (!isLocked) throw new Error("Slot already temporarily reserved, please choose another.");
