...
export interface IChatusecase {
    fetchChats(payload: { user_id: string; search: string; page: number; role: "lawyer" | "client" }): Promise<any>;
    getChatSessionById(sessionId: string): Promise<FetchChatSessionOutPutDto | null>;
    ...
    updateChatName(payload: { chatId: string; chatName: string }): Promise<ChatSession | null>;
    deleteMessage(payload: { messageId: string; sessionId: string }): Promise<ChatMessageOutputDto | null>;
    reportMessage(payload: { messageId: string; reason: string; reportedAt: Date }): Promise<void>;
