...
exports.FetchLawyerDashboardDataUsecase = void 0;
class FetchLawyerDashboardDataUsecase {
    _caseRepo;
    _appointmentRepo;
    _sessionRepo;
    _walletRepo;
    _walletTransactionRepo;
    _reviewRepo;
    _commissionTransactionRepo;
    constructor(_caseRepo, _appointmentRepo, _sessionRepo, _walletRepo, _walletTransactionRepo, _reviewRepo, _commissionTransactionRepo) {
        this._caseRepo = _caseRepo;
        this._appointmentRepo = _appointmentRepo;
        this._sessionRepo = _sessionRepo;
        this._walletRepo = _walletRepo;
        this._walletTransactionRepo = _walletTransactionRepo;
        this._reviewRepo = _reviewRepo;
        this._commissionTransactionRepo = _commissionTransactionRepo;
    }
    async execute(input) {
        ...
        const completedAppointments = sessions.filter((s) => s.status === "completed").length;
        ...
        const walletTransactions = wallet
            ...
            : [];
        const commissionTransactions = await this._commissionTransactionRepo.findByUserId(input);
        const walletBalance = wallet?.balance || 0;
        const totalEarnings = walletTransactions.reduce((acc, wt) => {
            return wt.type === "credit" ? acc + wt.amount : acc;
        }, 0);
        const totalCommissionPaid = commissionTransactions.reduce((acc, ct) => acc + ct.commissionAmount, 0);
        const upcomingList = appointments
            .filter((a) => new Date(a.date) > new Date() && a.status !== "cancelled")
            .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        const nextAppointment = upcomingList.length > 0
            ? {
        ...
        const earningsPrevMonth = walletTransactions
            ...
            .reduce((sum, t) => sum + t.amount, 0);
        const earningsGrowthPercent = earningsPrevMonth > 0
            ? ((earningsThisMonth - earningsPrevMonth) / earningsPrevMonth) * 100
            : 0;
        const casesThisMonth = cases.filter((c) => c.createdAt.getMonth() === currentMonth &&
            c.createdAt.getFullYear() === currentYear).length;
        const casesPrevMonth = cases.filter((c) => c.createdAt.getMonth() ===
            (currentMonth === 0 ? 11 : currentMonth - 1) &&
            c.createdAt.getFullYear() ===
                (currentMonth === 0 ? currentYear - 1 : currentYear)).length;
        const caseGrowthPercent = casesPrevMonth > 0
...
exports.FetchLawyerDashboardDataUsecase = FetchLawyerDashboardDataUsecase;
