...
export class UpdateAvailableSlotUseCase implements IUpdateAvailableSlotsUseCase {
    ...
    async execute(input: AvailabilityInputDto): Promise<AvailabilityOutputDto> {
        ...
        for (const [day, data] of Object.entries(input)) {
            ...
            const slots = data.timeSlots.map((slot: { start: string; end: string }) => ({
                ...slot,
            ...
            for (const slot of slots) {
                if (slot.startMin < 0 || slot.endMin > 1440) {
                    const error: any = new Error("Time should be between 00:00 and 23:59.");
                    error.code = STATUS_CODES.BAD_REQUEST;
                    throw error;
                }
                if (slot.startMin >= slot.endMin) {
                    const error: any = new Error("Start time should be less than end time.");
                    error.code = STATUS_CODES.BAD_REQUEST;
                    throw error;
                }
                if (Math.abs(slot.startMin - slot.endMin) < settings.slotDuration) {
                    const error: any = new Error(`Slot duration should be at least ${settings.slotDuration} minutes.`);
                    error.code = STATUS_CODES.BAD_REQUEST;
                    throw error;
                }
            }

            const sorted = slots.sort((a: { startMin: number }, b: { startMin: number }) => a.startMin - b.startMin);
